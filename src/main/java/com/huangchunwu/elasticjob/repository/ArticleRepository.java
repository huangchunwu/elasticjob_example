...
import com.huangchunwu.elasticjob.core.constants.FieldName;
...
public class ArticleRepository implements IArticleRepository {
    ...
    @Override
    public List<Article> findTodoData(String shardingParameter, int i) {
        Query query = Query.query(Criteria.where(FieldName.TAGS).in(shardingParameter).and(FieldName.PUSHFLAG).is(0));
        return mongoTemplate.find(query,Article.class);
    }
    ...
    public void setCompleted(String id) {
        Query query = Query.query(Criteria.where(FieldName.ID).is(id));
        Update update = Update.update(FieldName.PUSHFLAG, 1);
